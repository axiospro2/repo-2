...
from app.core.logging import bind_context, clear_context, setup_logging, get_logger, log_event

setup_logging()

app = FastAPI(title="Faturamento", version="1.0.0",
...
async def _contexto_invocacao(request: Request, call_next):
    ...
    log_event(_logger, "faturamento.requisicao.inicio", level="info",
        metodo=request.method, caminho=request.url.path, request_id=request_id)

    try:
        response = await call_next(request)
        latencia_ms = (time.time() - start_time) * 1000
        log_event(_logger, "faturamento.requisicao.fim", level="info",
            metodo=request.method, caminho=request.url.path,
            status_code=response.status_code, latencia_ms=round(latencia_ms, 2))
        return response
    except Exception as e:
        latencia_ms = (time.time() - start_time) * 1000
        _logger.exception("faturamento.requisicao.erro_middleware",
            extra={"ctx": {
...
app.include_router(router, prefix="/irb-cra-faturamento/v1")  # SALVAR (POST /faturamento/{documento})
app.include_router(router_buscar, prefix="/irb-cra-faturamento/v1") # BUSCAR (GET /faturamento/{documento}, /conglomerados/.../{subgr...
