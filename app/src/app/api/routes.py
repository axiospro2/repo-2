...
from app.api.deps import get_endpoint, get_parametros, get_repo
...
def salvar(
    ...
    racf: Optional[str] = Header(None, alias=settings.racf_header,
                                  description="RACF de quem informou (responsabilização)"),
    repo: DynamoRepository = Depends(get_repo),
    parametros: ParametrosClient = Depends(get_parametros),
    endpoint=Depends(get_endpoint),  # necessário para naoFoiEditado=true
) -> dict:
    ...
    salvo = service.salvar(
        fat, repo, parametros.obter(),
        endpoint=endpoint,
        metadados_compartilhados=body.metadados_por_subgrupo,
    )
